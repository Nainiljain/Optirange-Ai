...
export default function TripPlannerClient({
  ...
  const handleSaveFav = async () => {
    const addr  = showSaveFavForm === 'start' ? start : destination;
    const coord = showSaveFavForm === 'start' ? startCoords : destCoords;
    if (!addr || !coord || !favLabel.trim()) return;
    await saveFavouriteAction(favLabel.trim(), favType, addr, coord.lat, coord.lon);
    const newLoc: SavedLocation = { id: Date.now().toString(), label: favLabel.trim(), type: favType, address: addr, lat: coord.lat, lon: coord.lon };
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2 min-h-[32px] items-center">
                ...
                {savedLocs.find(l => l.type === 'home') ? (
                  ...
                  <button type="button" onClick={() => setShowSaveFavForm('start')}
              ...
              <div className="relative z-50">
                ...
                {start && (
                  <button type="button" onClick={() => setShowSaveFavForm('start')}
                    className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-foreground/30 hover:text-yellow-400 transition-colors" title="Save as favourite">
              ...
              <div className="relative z-40">
                ...
                />
                {destination && (
                  <button type="button" onClick={() => setShowSaveFavForm('dest')}
                    className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-foreground/30 hover:text-yellow-400 transition-colors" title="Save as favourite">
                    <Star className="w-4 h-4" />
                  </button>
                )}
              ...
              <AnimatePresence>
                {showSaveFavForm && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
                    ...
                    {/* Address input for saving */}
                    <input type="text" placeholder="Enter address to save…"
                    ...
                    <div className="flex gap-2">
                      {(['home','work','favourite'] as const).map(t => (
                        <button key={t} type="button"
                          onClick={() => { setFavType(t); setFavLabel(t === 'home' ? 'Home' : t === 'work' ? 'Work' : ''); }}
                          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold capitalize border transition-all ${favType === t ? 'bg-yellow-500/20 text-yellow-400 border-yellow-500/40' : 'border-border text-foreground/50 hover:border-yellow-500/30'}`}>
                          {t === 'home' ? <Home className="w-3 h-3" /> : t === 'work' ? <Briefcase className="w-3 h-3" /> : <Star className="w-3 h-3" />} {t}
                        </button>
                      ))}
                    </div>
                    <input type="text" placeholder="Label (e.g. Mom's House)"
                      value={favLabel} onChange={e => setFavLabel(e.target.value)}
                      className="w-full bg-background/50 border border-border rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-yellow-500" />
                    <div className="flex gap-2">
                      <button type="button" onClick={async () => {
                        ...
                        const coord = showSaveFavForm === 'start' ? startCoords : destCoords;
                        if (!addr || !favLabel.trim()) return;
                        // If no coords yet, geocode the address
                        ...
                        await saveFavouriteAction(favLabel.trim(), favType, addr, finalCoord.lat, finalCoord.lon);
                        const newLoc: SavedLocation = { id: Date.now().toString(), label: favLabel.trim(), type: favType, address: addr, lat: finalCoord.lat, lon: finalCoord.lon };
                        setSavedLocs(prev => favType === 'home' || favType === 'work'
                          ? [...prev.filter(l => l.type !== favType), newLoc]
                          : [...prev, newLoc]);
                        setShowSaveFavForm(null); setFavLabel(''); setFavType('favourite');
                      ...
                      <button type="button" onClick={() => setShowSaveFavForm(null)}
