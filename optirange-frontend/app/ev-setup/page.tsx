...
import {
  ...
  Search, ChevronDown, Loader2, CheckCircle2, Sparkles,
} from 'lucide-react'

const initialState = { error: '' }

// ─── CarAPI / EV Lookup Hook ───────────────────────────────────────────────────
...
function EvSetupContent() {
  ...
  const applySpec = (spec: any) => {
    ...
    // batteryCapacityKwh — use directly; if null, leave blank for manual entry
    if (spec.batteryCapacityKwh) setFBattery(String(spec.batteryCapacityKwh))
    // rangeKm from NREL (rangeMiles → km) or direct rangeKm from CarAPI
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div className="relative">
              ...
              <div className="relative">
                <select
                  value={lookupMake}
                  onChange={e => handleLookupMakeChange(e.target.value)}
            ...
            <div className="relative">
              ...
              <div className="relative">
                <select
                  value={lookupModel}
                  onChange={e => handleLookupModelChange(e.target.value)}
                  disabled={!lookupMake || loadingModels}
                  className="w-full appearance-none bg-background/50 border border-border rounded-xl px-3 py-2.5 pr-8 text-sm outline-none focus:ring-2 focus:ring-purple-500 font-medium transition-all disabled:opacity-50"
                >
          ...
          <AnimatePresence>
            ...
            {!loadingSpecs && lookupMake && lookupModel && specs.length === 0 && (
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                ...
                No EV specs found for this trim — please enter manually below.
              </motion.p>
            )}
          </AnimatePresence>
        ...
        <div className="glass-panel p-8 md:p-10 rounded-3xl shadow-2xl relative overflow-hidden">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold tracking-tight mb-2">Configure Your EV</h1>
            <p className="text-sm text-foreground/60 max-w-md mx-auto">
              Use the auto-fill panel above or enter your vehicle specs manually.
            </p>
          </div>
